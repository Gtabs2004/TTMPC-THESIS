...
class MembershipPaymentCreateRequest(BaseModel):
    ...
    processed_by_role: str | None = None
...
def build_membership_payment_id(sequence_number: int) -> str:
    return build_sequence_id("TTMPCMP-", sequence_number)


def normalize_membership_payment_type(raw_value: str | None) -> str:
    text = str(raw_value or "").strip().lower()
    if text in {"membership fee", "membership_fee", "membership"}:
        return "membership_fee"
    if text in {"paid_up_capital", "paid-up capital", "paid up capital", "initial_paid_up_capital"}:
        return "paid_up_capital"
    return text or "membership_fee"


def normalize_membership_payment_status(raw_value: str | None) -> str:
    text = str(raw_value or "").strip().lower()
    if text in {"pending", "pending_verification"}:
        return "pending_verification"
    if text in {"validated", "confirmed", "approved"}:
        return "validated"
    if text == "rejected":
        return "rejected"
    return "validated"
...
def resolve_membership_payment_amount(payment_type: str, amount: Decimal) -> Decimal:
    if payment_type == "membership_fee":
        return Decimal("100")
    if payment_type == "paid_up_capital":
...
@app.get("/api/cashier/membership-payments/members")
async def get_cashier_membership_payment_members():
    if not supabase:
        raise HTTPException(status_code=500, detail="Supabase client is not initialized.")
    ...
    try:
        members_response = (
            supabase.table("member")
            .select("id,membership_id,first_name,middle_initial,last_name,created_at,is_bona_fide")
            .order("created_at", desc=True)
            .execute()
        )
        members_rows = members_response.data or []
        ...
        for row in members_rows:
            ...
            members.append(
                {
                    "member_uuid": member_uuid,
                    "member_id": row.get("membership_id") or member_uuid,
                    "member_name": resolve_member_full_name(row),
                    "created_at": row.get("created_at"),
                    "is_bona_fide": bool(row.get("is_bona_fide")),
                }
            )
        ...
        try:
            applications_response = (
                ...
                .select("application_id,first_name,middle_name,surname,last_name,created_at,application_status")
                .order("created_at", desc=True)
                .execute()
            )
            for row in applications_response.data or []:
                ...
                full_name = " ".join(
                    ...
                    for part in [
                        ...
                        str(row.get("surname") or row.get("last_name") or "").strip(),
                    ]
                    if part
                ).strip()
                ...
                applicants.append(
                    {
                        ...
                        "member_name": full_name or "Unknown Applicant",
                        "created_at": row.get("created_at"),
        ...
        return {"success": True, "data": {"members": members, "applicants": applicants}}
    except Exception as err:
        raise HTTPException(status_code=500, detail=f"Failed to load membership payment members: {err}")


@app.get("/api/cashier/membership-payments/transactions")
async def get_cashier_membership_payments():
    if not supabase:
        raise HTTPException(status_code=500, detail="Supabase client is not initialized.")
    ...
    try:
        ...
        members_by_id = {str(row.get("id") or "").strip(): row for row in (members_response.data or [])}
        members_by_membership = {
            str(row.get("membership_id") or "").strip(): row for row in (members_response.data or [])
        }
        ...
        try:
            applications_response = (
                supabase.table("member_applications")
                .select("application_id,first_name,middle_name,surname,last_name")
                .execute()
            )
            for row in applications_response.data or []:
                app_id = str(row.get("application_id") or "").strip()
                if app_id:
                    applicants_by_id[app_id] = row
        except Exception:
            applicants_by_id = {}
        ...
        rows = payments_response.data or []
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            member_row = members_by_id.get(member_id) or members_by_membership.get(member_code)
            applicant_row = applicants_by_id.get(member_code)

            if member_row:
                member_name = resolve_member_full_name(member_row)
                member_display_id = member_row.get("membership_id") or member_id
            elif applicant_row:
                member_name = " ".join(
                    part
                    for part in [
                        str(applicant_row.get("first_name") or "").strip(),
                        str(applicant_row.get("middle_name") or "").strip(),
                        str(applicant_row.get("surname") or applicant_row.get("last_name") or "").strip(),
                    ]
                    if part
                ).strip() or "Unknown Applicant"
                member_display_id = member_code
            else:
                member_name = "Unknown Member"
                member_display_id = member_code or member_id

            mapped.append(
                {
                    "payment_id": row.get("payment_id") or build_membership_payment_id(idx),
                    "member_id": member_display_id,
                    "member_name": member_name,
                    "payment_type": row.get("payment_type") or "membership_fee",
                    "amount": decimal_to_float(row.get("amount") or 0),
                    "payment_status": row.get("payment_status") or "validated",
                    "payment_method": row.get("payment_method") or "Cash",
                    "reference_number": row.get("reference_number") or "",
                    "processed_by": row.get("processed_by") or "Cashier",
                    "payment_date": row.get("payment_date"),
                    "created_at": row.get("created_at"),
                }
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to load membership payments: {err}")


@app.post("/api/cashier/membership-payments")
async def create_cashier_membership_payment(payload: MembershipPaymentCreateRequest):
    if not supabase:
        raise HTTPException(status_code=500, detail="Supabase client is not initialized.")

    try:
        member_id = str(payload.member_id or "").strip() or None
        member_code = str(payload.member_code or "").strip() or None
        if not member_id and not member_code:
            raise HTTPException(status_code=400, detail="member_id or member_code is required.")

        payment_type = normalize_membership_payment_type(payload.payment_type)
        status_value = normalize_membership_payment_status(payload.payment_status)

        amount_value = resolve_membership_payment_amount(payment_type, Decimal(str(payload.amount or 0)))
        if amount_value <= 0:
            raise HTTPException(status_code=400, detail="amount must be greater than zero.")

        payment_count_response = supabase.table("membership_payments").select("id").execute()
        next_sequence = len(payment_count_response.data or []) + 1
        payment_id = build_membership_payment_id(next_sequence)

        processed_by = str(payload.processed_by or "").strip() or "Cashier"
        processed_by_role = str(payload.processed_by_role or "cashier").strip().lower() or "cashier"
        ...
        insert_payload = {
            ...
            "amount": decimal_to_float(money(Decimal(str(amount_value)))),
            "payment_status": status_value,
            "payment_method": str(payload.payment_method or "Cash").strip() or "Cash",
            "reference_number": str(payload.reference_number or "").strip() or None,
            "processed_by": processed_by,
            "processed_by_role": processed_by_role,
            "payment_date": (payload.payment_date or datetime.utcnow()).isoformat(),
        }

        insert_response = supabase.table("membership_payments").insert(insert_payload).execute()
        inserted_row = (insert_response.data or [None])[0]

        return {
            "success": True,
            "data": {
                "payment_id": (inserted_row or {}).get("payment_id") or payment_id,
                "member_id": member_code or member_id,
                "payment_type": payment_type,
                "amount": insert_payload["amount"],
                "payment_status": status_value,
                "payment_method": insert_payload["payment_method"],
                "reference_number": insert_payload["reference_number"],
                "processed_by": processed_by,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to record membership payment: {err}")


@app.get("/api/membership-approval/requirements/{application_id}")
async def get_membership_approval_requirements(application_id: str):
    if not supabase:
        raise HTTPException(status_code=500, detail="Supabase client is not initialized.")
    ...
    def _attendance_verified(app_id: str) -> bool:
        try:
            response = (
    ...
    try:
        ...
        membership_id = (app_response.data or [None])[0].get("membership_id") if app_response.data else None
    except Exception:
        membership_id = None
    ...
    filters = [f"member_code.eq.{clean_application_id}"]
    if membership_id:
        filters.append(f"member_code.eq.{membership_id}")
    if member_uuid:
        filters.append(f"member_id.eq.{member_uuid}")

    paid_up_payment = None
    paid_up_verified = False
    try:
        payment_response = (
            supabase.table("membership_payments")
            .select("payment_type,amount,payment_status,payment_method,reference_number,processed_by,payment_date")
            .or_(",".join(filters))
            .order("payment_date", desc=True)
            .execute()
        )
        for row in payment_response.data or []:
            payment_type = normalize_membership_payment_type(row.get("payment_type"))
            if payment_type != "paid_up_capital":
                continue
            status_value = normalize_membership_payment_status(row.get("payment_status"))
            amount_value = Decimal(str(row.get("amount") or 0))
            if status_value == "validated" and amount_value >= Decimal("10000"):
                paid_up_payment = row
                paid_up_verified = True
                break
    except Exception:
        paid_up_payment = None
        paid_up_verified = False

    attendance_verified = _attendance_verified(clean_application_id)

    return {
        "success": True,
        "data": {
            "attendance_verified": attendance_verified,
            "paid_up_capital_verified": paid_up_verified,
            "paid_up_payment": paid_up_payment,
        },
    }
