...
| Destructive/money-moving screens missing a confirm gate | 0 (fixed — see note below) |
...
| [Savings_Details.jsx](TTMPC_THESIS/src/Cashier/Components/Savings_Details.jsx) | Savings deposit / withdrawal modal submit | **RESOLVED.** The original finding here named Cashier_Withdrawals, Cashier_CBU, and Cashier_CBU_Deposit off a `grep -c useConfirm` count of 0, but that count was misleading: `Cashier_Withdrawals.jsx` is a read-only posted-transactions table (no submit at all), `Cashier_CBU.jsx`'s "Deposit" button only `navigate()`s to the entry page, and `Cashier_CBU_Deposit.jsx` already gated its POST at line 107. The genuine gap was `Savings_Details.jsx`, which posted both deposits and withdrawals straight to the savings ledger from a modal with only an amount field. | Gates added: deposit uses `tone: "default"`, withdrawal uses `tone: "warning"` and previews the resulting balance. Matches [Cashier_Payments.jsx:538](TTMPC_THESIS/src/Cashier/Components/Cashier_Payments.jsx#L538) and [Cashier_Disbursement.jsx:233](TTMPC_THESIS/src/Cashier/Components/Cashier_Disbursement.jsx#L233). | ✅ Fixed |
...
3. **✅ Confirm gates — done.** Added to `Savings_Details.jsx` (deposit + withdrawal), the one Cashier screen that actually posted money without one. The three screens originally named in this audit needed no change: two submit nothing, and `Cashier_CBU_Deposit.jsx` was already gated. Misclick test case still worth running against the savings modals.
