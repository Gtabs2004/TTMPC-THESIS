...
// Read-only oversight view — Treasurer doesn't post deposits/withdrawals
// (that's Cashier's teller function); this mirrors Bookkeeper's own
// Savings Accounts page, which is the same read + drill-in pattern.
const Treasurer_Savings_Accounts = () => {
  ...
  // Reset to page 1 whenever the filter criteria change, adjusted during
  // render rather than in an effect (avoids the extra render-then-reset
  // cascade of a setState-in-effect).
  const filterKey = `${searchTerm}|${kindFilter}|${sortBy}`;
        ...
        <main className="p-8">
          <div className="mb-6">
            <div className="flex items-center justify-between gap-4 mb-4">
              <div>
                <Breadcrumb portal="Treasurer" page="Savings Accounts" />
                <h1 className="text-xl font-bold text-gray-900">Savings Accounts</h1>
                <p className="text-xs text-gray-500 mt-0.5">
                  Read-only oversight of member passbooks and standalone cooperative funds
                </p>
              </div>
                  ...
                  {status === "loading" ? (
                    ...
                    paginated.map((row) => (
                      <tr
                        ...
                        onClick={() =>
                          navigate(`/treasurer-savings-accounts/${encodeURIComponent(row.account_number)}`)
                        }
                      >
                        ...
                        >
                          <TableActionButton
                            onClick={() =>
                              navigate(
                                `/treasurer-savings-accounts/${encodeURIComponent(row.account_number)}`
                              )
                            }
                          >
                            View
...
export default Treasurer_Savings_Accounts;
