  ...
  */
  ...
  const goToNextMobileStep = () => {
    if (validateMobileStep(mobileStep)) {
      setMobileStep((step) => Math.min(step + 1, mobileSteps.length - 1));
    }
  };
  ...
  const handleFormSubmit = (e) => {
    ...
    if (mobileStep < mobileSteps.length - 1 && window.matchMedia('(max-width: 767px)').matches) {
      goToNextMobileStep();
      return;
    }
            ...
            {collaterals.length === 0 ? (
              <div className="border-2 border-dashed border-gray-200 rounded-xl px-6 py-10 flex flex-col items-center text-center bg-gray-50/60">
                ...
                <button
                  ...
                  className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-member-green px-4 py-2 text-xs font-bold text-white hover:bg-[#154a19] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-member-green transition-colors"
                >
                          ...
                          >
                            ...
                            {collateralUploadingIndex === index && (
                              <span className="absolute inset-0 flex items-center justify-center bg-white/70">
                                <Loader2 className="h-5 w-5 animate-spin text-member-green" aria-hidden="true" />
                              </span>
                            )}
