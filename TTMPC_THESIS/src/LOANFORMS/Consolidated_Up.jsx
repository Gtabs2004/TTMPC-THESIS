...
// Options strictly starting from 500,000 up to 1,500,000 in 10k increments
const CONSOLIDATED_LOAN_AMOUNT_OPTIONS = Array.from(
  { length: ((1500000 - 500000) / 10000) + 1 },
  (_, index) => String(500000 + (index * 10000))
);
...
function Consolidated_Up() {
  ...
  const RENEWAL_DEDUCTIONS = 0;  
  const MIN_PAID_MONTHS_FOR_RENEWAL = 6;
  const MONTHLY_INTEREST_FACTOR = 0.0083;
  
  // Extended terms for > 500k include 72 months based on PDF Table
  ...
  // OVER-500K POLICY GATES
  const actualNetPay = Number(formData.latest_net_pay || 0);
  const actualShareCapital = Number(formData.share_capital || 0);
  const isPolicyCompliant = actualNetPay >= 20000 && actualShareCapital >= 100000;
  ...
  const dropdownLoanCapacity = (() => {
    const shareCapital = Number(formData.share_capital || 0);
    if (!shareCapital) return Infinity; 
  ...
  const submissionBlockMessage =
    !isPolicyCompliant ? 'Minimum ₱100,000 Share Capital and ₱20,000 Net Pay required.' :
    ...
    <div className="flex flex-col min-h-screen bg-gray-100 pb-44">
      ...
      <form ref={formRef} noValidate={window.matchMedia('(max-width: 767px)').matches} onSubmit={handleFormSubmit} className="px-3 sm:px-4">
        <section className="grid gap-6 px-3 sm:gap-8 sm:px-4">
          <h1 className="mt-8 text-center text-xl font-bold text-primary sm:mt-12 sm:text-2xl">CONSOLIDATED LOAN APPLICATION (Over ₱500,000)</h1>
            ...
            <div className="bg-[#EEF6F1] rounded-xl p-4 border-2 border-primary flex flex-col items-start justify-between gap-4 sm:p-6 md:flex-row md:items-center md:gap-6">
              <div className="flex flex-wrap gap-4 sm:gap-8">
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="application_type" value="New" checked={formData.application_type === 'New'} onChange={handleChange} className="h-4 w-4 accent-primary" />
                  <span className="font-semibold text-gray-700">New</span>
                </label>
                <label className="flex items-center space-x-2 cursor-pointer">
                  <input type="radio" name="application_type" value="Renewal" checked={formData.application_type === 'Renewal'} onChange={handleChange} className="h-4 w-4 accent-primary" />
        ...
        {/* OVER 500k POLICY WARNING BANNER */}
        {!isPolicyCompliant && actualNetPay > 0 && actualShareCapital > 0 && (
          <div className="mt-6 max-w-6xl mx-auto w-full px-4">
             <div className="rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-700 font-semibold shadow-sm">
                Policy Restriction: Consolidated loans over ₱500,000 require a minimum Share Capital of ₱100,000 and a minimum Net Pay of ₱20,000.
             </div>
          </div>
        )}
        ...
        <div data-mobile-step="0" className={`${mobileStep === 0 ? 'block' : 'hidden'} mobile-step-transition md:block mt-10 bg-white rounded-lg shadow-md overflow-hidden max-w-6xl mx-auto w-full`}>
          ...
          <div className="p-4 sm:p-6 lg:p-8 grid grid-cols-1 md:grid-cols-3 gap-y-8 gap-x-6 md:gap-6">
            ...
            <div>
              ...
              <div className="relative"><span className={`absolute left-3 top-2 text-xs ${previewNetPay > 0 && previewNetPay < 20000 ? 'text-red-700' : 'text-gray-400'}`}>₱</span><input type="number" name="latest_net_pay" value={formData.latest_net_pay} onChange={handleChange} className={`${inputClassFor('latest_net_pay')} pl-7 ${!isLockedField('latest_net_pay') && previewNetPay >= 20000 && eligibilityCardData.eligibilityPass ? 'bg-[#E9F7DE] border-primary' : (previewNetPay > 0 && previewNetPay < 20000 ? 'bg-red-50 border-red-300 text-red-700' : '')}`} required {...lockedInputProps('latest_net_pay')} /></div>
            </div>
            <div>
              <label className={labelStyles}>Share Capital <span className="text-red-500">*</span></label>
              <div className="relative"><span className={`absolute left-3 top-2 text-xs ${previewShareCapital > 0 && previewShareCapital < 100000 ? 'text-red-700' : 'text-gray-400'}`}>₱</span><input type="number" name="share_capital" value={formData.share_capital} onChange={handleChange} className={`${inputClassFor('share_capital')} pl-7 ${!isLockedField('share_capital') && previewShareCapital >= 100000 && Number.isFinite(dropdownLoanCapacity) ? 'bg-[#E9F7DE] border-primary' : (previewShareCapital > 0 && previewShareCapital < 100000 ? 'bg-red-50 border-red-300 text-red-700' : '')}`} required {...lockedInputProps('share_capital')} /></div>
            </div>
       ...
       {/* Section 2: LOAN AGREEMENT */}
          ...
          <div className="p-4 sm:p-6 lg:p-8 text-sm text-gray-800">
            ...
            <div className="leading-relaxed sm:leading-[3.5rem]">
              ...
              <select 
                ...
                onChange={handleChange} 
                className="border border-gray-300 rounded-md px-3 py-1.5 focus:ring-2 focus:ring-primary outline-none bg-white text-sm transition-all mx-0 my-2 w-full sm:mx-2 sm:my-0 sm:w-64 inline-block align-middle"
              >
                      ...
                      <div className="md:col-span-2">
                        ...
                        {row.document_url && collateralPreviews[row.document_url] ? (
                          ...
                          <button
                            type="button"
                            onClick={() => collateralFileInputRefs.current[index]?.click()}
                            className="inline-flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-2.5 py-2 text-xs font-semibold text-gray-700 hover:border-primary hover:text-[#1D6021] transition-colors"
            ...
            {collaterals.length > 0 && (
              <div className="mt-5">
                <button
                  type="button"
                  onClick={addCollateral}
                  className="w-full inline-flex items-center justify-center gap-1.5 rounded-lg border-2 border-dashed border-gray-300 bg-white px-4 py-3 text-xs font-bold text-gray-600 hover:border-primary hover:text-[#1D6021] hover:bg-[#F5FBF0] transition-colors"
                >
        ...
        <div data-mobile-step="2" className={`${mobileStep === 2 ? 'block' : 'hidden'} mobile-step-transition md:block mt-8 bg-white rounded-lg shadow-md overflow-hidden max-w-6xl mx-auto w-full`}>
          ...
          <div className="p-4 sm:p-6 lg:p-8 text-sm text-gray-800">
            <div className="leading-[3.5rem]">
              ...
              <span className="block mt-2 leading-normal">
                through: (1) deduction from share capital; (2) by my co-makers; (3) value of collateral (if there is any); (4) deduction from any benefit from employment.
              </span>
            </div>
        ...
        <div data-mobile-step="3" className={`${mobileStep === 3 ? 'block' : 'hidden'} mobile-step-transition md:hidden mt-8 rounded-lg border border-gray-200 bg-white p-4 shadow-sm`}>
          ...
          <button type="submit" disabled={loading || printing || exceedsCeiling || renewalBlocked || eligibilityFailed || stressIndexExceeded || !isPolicyCompliant} className="mt-5 w-full rounded bg-primary-deep px-6 py-2.5 text-sm font-bold text-white transition-colors hover:bg-member-green disabled:cursor-not-allowed disabled:opacity-50">
...
export default Consolidated_Up;